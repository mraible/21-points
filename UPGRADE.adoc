...
- After upgrading, there's an error when creating new records. I was able to work around this by removing the `<not>` from the precondition in the `migrate_from_v5.xml` migration.
